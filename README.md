...
├── models/                # Serialized `.pkl` ML models (⚙️ auto-generated by run_project.py)
└── reports/               # Markdown and text reports
```

> **Note:** The `models/` folder and its `.pkl` files are automatically generated when you run `python run_project.py`. They are listed in `.gitignore` and do not need to be committed to version control.
...
The dataset used in this project is the **Healthcare Dataset** by Prasad22, available on Kaggle:

> 🔗 **[Healthcare Dataset — Kaggle](https://www.kaggle.com/datasets/prasad22/healthcare-dataset)**

- **Records**: 54,966 patients
- **Features**: 15 raw columns (expanded to 35 after feature engineering)
- **Type**: Synthetic Electronic Health Record (EHR) data
- **Download**: Place the downloaded `healthcare_dataset.csv` in `data/raw/` before running the pipeline.
